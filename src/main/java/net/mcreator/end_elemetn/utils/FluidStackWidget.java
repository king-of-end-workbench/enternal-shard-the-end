 ...
 */
package net.mcreator.end_elemetn.utils;
			...
			if (Minecraft.getInstance().getTextureManager().getTexture(InventoryMenu.BLOCK_ATLAS) instanceof TextureAtlas atlas) {
				...
				for (int i = 0; (double) i < Math.ceil(renderableHeight / 16.0); i++) {
					...
					for (int j = 0; (double) j < Math.ceil(this.getWidth() / 16.0); j++) {
						...
						guiGraphics.blit(InventoryMenu.BLOCK_ATLAS, this.getX() + 16 * j, this.getY() + yOffset, 0, sprite.getU0() * atlasWidth, sprite.getV0() * atlasHeight + (float) yOffset, renderWidth, renderHeight, (int) atlasWidth,
								(int) atlasHeight);
					}
