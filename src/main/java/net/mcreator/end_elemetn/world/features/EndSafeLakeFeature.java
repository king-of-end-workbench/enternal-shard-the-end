...
/**
 * Copy of vanilla LakeFeature.place() with the surface freeze pass removed. That pass calls
 * level.getBiome() for every column above the lake, which in this dimension's floating-island
 * layout can reach a neighboring chunk that isn't generated far enough yet and throws
 * "Requested chunk unavailable during world generation". The End never freezes lakes anyway, so
 * the check has no gameplay effect here - dropping it removes the only unsafe call in the feature.
 */
public class EndSafeLakeFeature extends Feature<LakeFeature.Configuration> {
	...
	public static final RegistryObject<Feature<?>> SAFE_LAKE = REGISTRY.register("safe_lake", () -> new EndSafeLakeFeature(LakeFeature.Configuration.CODEC));

	private static final BlockState AIR = Blocks.CAVE_AIR.defaultBlockState();

	public EndSafeLakeFeature(Codec<LakeFeature.Configuration> codec) {
		super(codec);
	}
